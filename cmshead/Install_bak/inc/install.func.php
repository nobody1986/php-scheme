...
function writable($var){
	$writeable = FALSE;
	$var = CMSHEAD_ROOT.$var;
	if(!is_dir($var)){
		@mkdir($var, 0777);
	}
	if(is_dir($var)){
		$var .= '/temp.txt';
		if(($fp = @fopen($var, 'w')) && (fwrite($fp, 'CMSHEAD'))){
			fclose($fp);
			@unlink($var);
			$writeable = TRUE;
		}
...
function clear(){
	$file['cache'] = array(CMSHEAD_ROOT."Admin/Runtime/Cache",CMSHEAD_ROOT."cmsheadApp/Runtime/Cache");
	$file['data'] = array(CMSHEAD_ROOT."Admin/Runtime/Data/_fields",CMSHEAD_ROOT."cmsheadApp/Runtime/Data/_fields");
	$file['temp'] = array(CMSHEAD_ROOT."Admin/Runtime/Temp",CMSHEAD_ROOT."cmsheadApp/Runtime/Temp");
	$file['logs'] = array(CMSHEAD_ROOT."Admin/Runtime/Logs",CMSHEAD_ROOT."cmsheadApp/Runtime/Logs");
	$file['runtime'] = array(CMSHEAD_ROOT."Admin/Runtime/~runtime.php",CMSHEAD_ROOT."cmsheadApp/Runtime/~runtime.php");
	$file['app'] = array(CMSHEAD_ROOT.CMSHEAD_ROOT."Admin/Runtime/~app.php",CMSHEAD_ROOT."cmsheadApp/Runtime/~app.php");
