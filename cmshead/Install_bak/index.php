...
error_reporting(0);
session_start();
define('CMSHEAD_INSTALL', TRUE);
define('CMSHEAD_ROOT', str_replace('\\', '/', substr(dirname(__FILE__), 0, -7)));
...
$timestamp				=	time();
$ip						=	getip();
$database			    =	'inc/cmshead.sql';
$cmshead_config_file     =   'config.php';

header('Content-Type: text/html; charset=utf-8');
if(file_exists(CMSHEAD_ROOT."/Install/install.lock")){
...
switch($step){
	...
	case 4:
		...
		//写配置文件
		$fp = fopen(CMSHEAD_ROOT.$cmshead_config_file, 'wb');
...
return array(
	...
	'CMSHEAD_URL'     =>  '$data[site_url]',// 网站地址
...
?>
<?php
	chmod(CMSHEAD_ROOT.$CMSHEAD_config_file, 0777);
	...
	case 5:
		...
		if(!$db_config['db_host'] && !$db_config['db_name']){
			...
			if($sqlv < '4.1'){
				...
				if(intval($link[0]) > 0){
					$CMSHEAD_rebuild	=	true;
					$msg .= $lang['cmshead_rebuild'];
					$alert = 'onclick="confirm(\''.$lang['cmshead_rebuild'].'\');window.location.href=\'index.php?step=6\'"';
				}else{
