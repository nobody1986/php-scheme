...
class TagLibCx extends TagLib {
    ...
    public function _volist($attr,$content) {
        ...
        //对ch2函数的支持 除了大于符号必须是&gt;以及表达式内的条件必须是双引号以外，其他没什么特别要求   awen edit 2013-03-30 
        //支持类似写法 <volist name=":ch2('news','where:find_in_set("2",attrtj) and status='.$a.' and status=1 and img!="" and title like "%中国商人%" and id&gt;='.$a,'limit:1')" id="vo">     
        if(preg_match('/^(name=[\'"])(:.+)([\'"].+?id=[\'"]\w+[\'"])$/i', $attr, $arr)){
        	$strparam = htmlspecialchars($arr[2]);
        	$attr = $arr[1].$strparam.$arr[3];
        } 
