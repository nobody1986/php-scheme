...
abstract class Action {
   ...
   /**
    ...
    protected function ajaxReturn($data,$type='') {
        ...
        $data['statusCode']  =  $data['status'] ? $data['status'] : 0;	// awen
        $data['navTabId']  =  $_REQUEST['navTabId'] ? $_REQUEST['navTabId'] : MODULE_NAME;	// awen
        if(empty($type)) $type  =   C('DEFAULT_AJAX_RETURN');
        switch (strtoupper($type)){
            case 'JSON' :
            	if( APP_NAME=='Admin' && $_REQUEST['is_iframe'] ){ //awen add for dwz
            		$data_info = trim(json_encode($data['info']),'"\'');
            		if($data['status']){
