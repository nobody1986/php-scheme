...
class Model {
    ...
    public function buildSql($options=array()) {
        ...
        $options =  $this->_parseOptions($options);
        //return  '( '.$this->db->buildSelectSql($options).' )';
        return  $this->db->buildSelectSql($options); //awen edit 整个SQL语句不必要多余的括号，否则不方便再次利用
    }
        ...
        switch(strtolower(trim($val[4]))) {
            ...
            case 'unique': // 验证某个值是否唯一
                ...
                if(is_array($val[0])) {
                    // 支持多个字段验证
                    foreach ($val[0] as $field)
                        $map[$field]   =  array('eq',$data[$field]); //$data[$field] //awen edit
                }else{
                    $map[$val[0]] = array('eq',$data[$val[0]]); //$data[$val[0]] //awen edit
                }
