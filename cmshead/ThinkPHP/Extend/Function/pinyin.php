...
function __U2_Utf8_Gb($_C,$charset) {
	$_String = '';
	if ($_C < 0x80)
		$_String .= $_C;
	elseif ($_C < 0x800) {
	...
	return iconv ( $charset, 'gbk', $_String );
} 

/**
 * 汉字字串转换成拼音，或者首字符， 中英文字符混杂可用
 * @param $string 
 * @param $isfirst 是否首字母
 * @param $charset 默认utf8
 * @return array
 */
function pinyin($string,$isfirst=false,$charset='utf-8') {
	if ($charset != 'gbk') $string = __U2_Utf8_Gb ( $string,$charset );
	$output = array();
	for($i = 0; $i < strlen ( $string ); $i ++) {
		...
		$fullword = __Dictionary ( $letter );
		$output[] = $isfirst ? substr($fullword,0,1) : $fullword;
	}
	return $output;
}
?>