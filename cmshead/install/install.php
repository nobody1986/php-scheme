...
<div class="contain">
	...
	<div class="body">
		<div class="lcs">
			...
			$sql_content="";
			$file = fopen("../data/bak/cmshead.sql","r"); //以只读方式打开数据库sql文件
			while($s = fread($file,1024)){
				$sql_content.=$s;
			}
			fclose($file);
			echo "<p>成功读取数据库信息文件...</p>";
			$sql_content=str_replace('ch_','',$sql_content);//将ch_替换成空	
			//4. 解析配置文件。形成建表语句数组
			...
			//7。添加后台管理员账户信息
			$sql = "insert into {$db_pre}user(account,password,nickname,bind_account,verify,remark,type_id,info,last_login_time,last_login_ip,email,create_time,status) values('{$account}','".md5($password)."','{$nickname}','','','',9,'','{$last_login_time}','{$last_login_ip}','{$email}','{$create_time}','{$status}')";
...
return array(
	...
	*/
	...
	//),';
		...
		//9 生成项目配置文件config.php
		file_put_contents('../Home/Conf/config.php','<?php
$config	=	require \'./config.php\';
$home_config =  array(
	\'APP_FILE_CASE\'			=> true,			// 是否检查文件的大小写 对Windows平台有效
	\'TMPL_CACHE_ON\'			=> false, 		//开启模板缓存
	\'URL_CASE_INSENSITIVE\'  => true, 		//URL不区分大小写
	'.($is_mod_rewrite?'':'//').'\'URL_MODEL\'             => 2,           //服务器开启Rewrite模块时，可去除URL中的index.php 参数模式1,2,3
...
return array_merge($config,$home_config);');			
			echo "<p>前台项目配置文件/Home/Conf/config.php配置成功！....</p>";
...
require(\'ThinkPHP/ThinkPHP.php\');   // 加载入口文件');
		...
		echo "<p>前台项目主入口文件/index.php配置成功！....</p>";
			...
			echo "<p>安装成功！....</p>";
			//12. 清除缓存
			@deleteDir('../Home/Runtime');
			@deleteDir('../Admin/Runtime');
			define('CMS_SELF','../');
