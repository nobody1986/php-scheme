//cmshead 若需要升级此js包,请搜索awen,手工酌情修改相同部分
var DWZ={
keyCode:{
...
layoutH: function($refBox) { //awen edit 
    return this.each(function() {
        var $this = $(this);
        if (!$refBox) $refBox = $this.parents("div.layoutBox:first");
        var iRefH = $refBox.height();
		var mainBodyH = (document.documentElement || document.body).clientHeight-$("#header").height()-$("#footer").height()-15;
		if($('.formBar').height()){
			 mainBodyH =  mainBodyH-$('.formBar').height();
		}
		if($('.tabsPageHeader').height()){
			 mainBodyH =  mainBodyH-$('.tabsPageHeader').height();
		}
		if($('.header').height()){
			mainBodyH =  mainBodyH-$('.header').height();
		}
		if(iRefH>mainBodyH) iRefH = mainBodyH - $(".panelBar").height() - 30; 
        var iLayoutH = parseInt($this.attr("layoutH"));
		var iH = iRefH - iLayoutH > 50 ? iRefH - iLayoutH: 50;
        if ($this.isTag("table")) {	
           if($this.parents("div.temp__layoutH").length<1) $this.removeAttr("layoutH").wrap('<div class="temp__layoutH" style="overflow:auto;height:' + iH + 'px"></div>');
        } else {
...
var jmenus=new Map();
$.dwz=$.dwz||{};
$(window).resize(function(){setTimeout(function(){for(var i=0;i<jmenus.size();i++){fillSpace(jmenus.element(i).key)}},100)});
$.fn.extend({
...
var $this=$(this);
var allPlugin={};/**插件**/
var pagetag=$this.attr("pagetag");
if(pagetag!=undefined){
	allPlugin.CMSHeadMenu = {c:'',t:'CMSHead功能菜单',h:1,e:function(){
		var _this=this;
		var arrMenu=[];
		arrMenu[0] = {s:'插入分页符',v:'[CHPAGE]',t:'内容页分页标志'};
		arrMenu[1] = {s:'清空分页符',v:':clearchpage',t:'内容页分页标志'};
		_this.saveBookmark();
		_this.showMenu(arrMenu,function(v){
				switch(v){
					case ':clearchpage':
						var Source=_this.getSource();
						if(Source.indexOf('[CHPAGE]')>-1) _this.setSource(Source.replace(/\[CHPAGE\]/g,''));
						break;
					default:
						if(_this.doc.selection){ //ie
							switch(_this.doc.selection.type){
								case 'Control':	
								//case 'Text':	
									var objFocus=_this.getRng()(0);
									if(objFocus.parentElement){
										$(objFocus.parentElement).after(v);
										$(objFocus).remove();
									}else{
										$(objFocus).html(v);
									}
									break;
								default:
									$(_this.getRng().parentElement()).after(v);
									break;	
							}							
						}else{ //ff
							_this.pasteHTML(v);
...
var op={html5Upload:true,skin:'default',width:$this.attr("width")||'100%',height:$this.attr("height")||'300',tools:$this.attr("tools")||'full',plugins:allPlugin};
...
findObj:function(str){//awen add 查找弹窗中存在的对象，从当前-父级-父父级顺序查找。返回弹窗对象和查找的对象 str示例 'input[name=template]'
	for(i=1;i<=10;i++){
		obj = $('.dialog').eq(-i).find(str);
		if(obj[0]){
			return {panel: $('.dialog').eq(-i), fobj: obj};
		}
	}
	obj = $(str, navTab.getCurrentPanel());
	if(!obj[0]) obj = $(str); return {panel: $(navTab.getCurrentPanel()), fobj: obj};
},
reload:function(url,options){
...
function _doPost(){
//{awen edit 多选的时候，支持弹出层或者打开一个页面，为了里面能够对多选值进行更多处理。
var showType=$this.attr("showType");
if(showType){
	var Title = $this.attr("alt") || $this.text();
	var external=eval($this.attr("external")||"false");
	var options = {};
	options.fresh = eval($this.attr("fresh") || "true");
	var paramsURL = $this.attr('href');
	paramsURL += (paramsURL.indexOf('?')>-1 ? '&' : '?')+selectedIds+'='+ids;
	if(showType=='dialog'){
		var w = $this.attr("width");
		var h = $this.attr("height");
		if (w) options.width = w;
		if (h) options.height = h;
		$.pdialog.open(paramsURL, selectedIds, Title, options);	
	}else if(showType=='navTab'){
		navTab.openTab(selectedIds,paramsURL,{title:Title,fresh:options.fresh,external:external});
		event.preventDefault();
	}
	return;	
}//}
