...
if(json.status===undefined&&json.info===undefined){
if(alertMsg)return alertMsg.error(json);
else return alert(json);}
if(json.status==DWZ.statusCode.error){
if(json.info&&alertMsg)alertMsg.error(json.info);}else if(json.status==DWZ.statusCode.timeout){
...
if(json.status==DWZ.statusCode.timeout){
...
var $this=$(this);
var op={html5Upload:true,skin:'default',width:$this.attr("width")||'100%',height:$this.attr("height")||'300',tools:$this.attr("tools")||'full'};
var upAttrs=[["upLinkUrl","upLinkExt","zip,rar,txt,doc,docx,ppt,xls,xlsx,csv"],["upImgUrl","upImgExt","jpg,jpeg,gif,png,bmp"],["upFlashUrl","upFlashExt","swf,flv,fla"],["upMediaUrl","upMediaExt","avi,wmv,wma,rm,mov,mpg,rmvb,3gp,mp4,mp3"]];
...
if(json&&json.status==DWZ.statusCode.timeout)this.closeCurrentTab();},
...
if(json&&json.status==DWZ.statusCode.timeout)this.closeCurrent();},
...
var args=$pagerForm.size()>0?$pagerForm.serializeArray():{}
navTabPageBreak(args,json.rel);}
//if("closeCurrent"==json.callbackType){$.pdialog.closeCurrent();} //awen
