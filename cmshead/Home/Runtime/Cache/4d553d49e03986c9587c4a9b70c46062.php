...
<div class="wrap">
    ...
    <div class="wrap_left">
        <div class="rbox5">
            ...
            <link rel="stylesheet" type="text/css" media="screen" href="__PUBLIC__/Plugin/Jplayer/mplayer.css"/><!--音乐播放CSS--> 
            
            <div id="mp_container">
                <div id="jquery_jplayer"></div> 
                <div id="player_title">
                    <h1>
                    <?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><span><a href="<?php echo ($vo["url"]); ?>"><?php echo ($vo["title"]); ?></a></span><?php endforeach; endif; else: echo "" ;endif; ?>
                    </h1>
                </div>
                    ...
                    <div class="playlist_main">
                        <div class="playlist_wrap" >
                            ...
                            <ul class="playlist_content" > 
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="player" jplayer="<?php echo ($vo["url"]); ?>" songer="<?php echo ($vo["author"]); ?>" ablum="<?php echo ($vo["title"]); ?>" songimg="">		
                                    <a href="#" class="controllink" ></a>
                                    <p><?php echo ($vo["title"]); ?></p>
                                    <span style=" position:absolute; left:350px; top:5px"><?php echo ($vo["author"]); ?></span>
                                    <span style=" position:absolute; left:450px; top:5px"><?php echo (getcategoryname($vo["tid"])); ?></span>
                                    <span style=" position:absolute; left:550px; top:5px"><?php echo (getusername($vo["adder_id"])); ?></span>
                                </li><?php endforeach; endif; else: echo "" ;endif; ?>
                            </ul>
                            </div> 
                        </div>
            ...
            <script type="text/javascript" src="__PUBLIC__/Plugin/Jplayer/mplayer.js"></script><!--音乐播放JS-->	
            </div>
        ...
        <div class="rbox5">
            ...
            <div class="rbox5_center">
                ...
                <div class="sub_form">
                        <div>
                            <select name="tid" id="tid"><?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?></select>
                            <label>音乐类型</label>
                        </div>
                        <div>
                            <input type="text" name="title" id="title" class="long" />
                            <label class="required">音乐名（必须）</label>
                        </div>
                        <div>
                            <input type="text" name="author" id="author" class="long" />
                            <label class="required">作者（必须）</label>
                        </div>
                        <div><input type="text" name="url" id="url" class="long" />
                            <label class="required">链接地址（必须）</label>
                        </div>
                        <div>
                            <table><tr><td>
		...
		function postData(){
			...
			$.post("__URL__/add", {"verify":$("#verify").val(),"title":$("#title").val(),"author":$("#author").val(),"url":$("#url").val(),"tid":$("#tid").val()}, function(msg) {
				$("#postTemp").html(msg);
				$("#verify").val('');
				$("#title").val('');
				$("#url").val('');
