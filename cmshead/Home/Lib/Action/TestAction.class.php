...
class TestAction extends CommonAction{
	...
	public function view($id=0){
		...
		$info = M('Test')->where("id=$id{$status}")->join(C('DB_PREFIX').'category ON '.C('DB_PREFIX').'category.classid = '.C('DB_PREFIX').'test.tid')->find();		
		$info or $this->error('没有这条记录！');
		M('Test')->where("id=$id")->setInc('apv'); //浏览量
		//分页处理
		$strpages = '';
		if(false!==strpos($info['content'],'[CHPAGE]')){
			...
			$info['content'] = preg_replace('/(^(\s*|(&nbsp;)*)<\/\w+?>)|(<\w+?>(\s*|(&nbsp;)*)<\/\w+?>)/','',$pages[$pageNum-1]); //去掉半截子html代码
			//  /news/?p=1  or /index.php/Test/view/id/12/?p=3
		...
		$pre = D('Test')->where("id<$id AND status=1")->order('id DESC')->field('id,title,apv,outurl,rewrite')->find();
		$pre['method']=$info['method'];
		$pre = changurl($pre);
		$this->assign('pre',$pre);//上一篇
		
		$next = D('Test')->where("id>$id AND status=1")->order('id')->field('id,title,apv,outurl,rewrite')->find();
		$next['method']=$info['method'];
		$next = changurl($next);
		$this->assign('next',$next);//下一篇
		
		$rand = D('Test')->where("status=1")->order('rand()')->limit(8)->select();
		...
		$message = D('Message')->where("modelname='test' and modekeyvalue=$id AND status=1 AND pid=0")->select();
		if(is_array($message)){
